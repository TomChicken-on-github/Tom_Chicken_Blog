...
**Scoop** 是 Windows 上最舒适的开源**命令行包管理器**。它可以方便地安装、管理和更新各种常用的软件和工具，彻底告别“找官网 -> 下包 -> 狂点下一[...]”的繁琐流程。
...
### Scoop 的目录结构
Scoop 安装路径下包含多个重要子目录，每个子目录的作用如下：

- `apps`：存储所有已安装的软件，每个软件对应一个子目录。
- `buckets`：存放已添加的软件桶数据，用于管理软件源。
- `cache`：用于存储下载的软件安装包缓存。
- `persist`：保存需要跨版本保留的文件或配置数据，避免软件更新后数据丢失。
- `shims`：包含 Scoop 自动生成的命令行快捷方式，便于快速调用已安装的软件。

通过这些目录的分工，Scoop 可以轻松实现软件安装、更新和删除的独立性与灵活性，同时保证环境的清洁与安全。
