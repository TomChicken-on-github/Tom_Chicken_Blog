...
│   └── [app-name]/
│       ├── [version]/  # 具体版本的文件
│       └── current/    # 软链接，指向当前正在使用的版本
├── buckets/     # 软件源仓库（Git Repo）
├── cache/       # Scoop 安装软件时产生的缓存，存放软件原始安装包（.zip / .exe）
├── persist/     # 持久化数据（软件配置文件、插件），即使某个版本被删除也能保留配置文件
└── shims/       # 环境变量入口（所有的 .exe 软链接）
```

 - `apps/` & `current/`  
   Scoop 管理的软件并非直接散落在目录下，而是按版本存放。current 目录是一个**符号链接**，始终指向最新的版本文件夹。当你升级软件时，Scoop 只需要更新这个指向，而不需要修改环境变量，降低了系统耦合度。
 - `shims/` **环境入口**  
   唯一需要添加到系统 PATH 的目录。当你运行一个命令时，调用的是 shim 目录下文件，再由 shim 转发给 apps/current 下的真实程序。
 - `persist/` **数据持久化**  
   Scoop 通过创建目录连接，将软件本应写入 AppData 或程序目录的配置重定向到改目录。当你卸载或升级软件时，个人数据和配置会原封不动地保留。
 - `buckets/` **桶清单**  
   这里存放的是各个软件仓库的本地镜像（JSON 文件）
